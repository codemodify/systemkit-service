...
var logTagSystemV = "SystemV-CONVERT"

// SERVICEToSystemV -
func SERVICEToSystemV(serviceSpec spec.SERVICE) (platformService string) {
	// for SystemV move everything into serviceSpec.Executable
...
# Default-Stop:      0 1 6# For RedHat and cousins:
# chkconfig: - 99 01
# description: {{.Description}}
# processname: {{.Executable}}
...
`))
	...
	if err := fileTemplate.Execute(&buffer, serviceSpec); err != nil {
		logging.Errorf("%s: error generating file: %s, from %s", logTagSystemV, err.Error(), helpersReflect.GetThisFuncName())
		return ""
	}
